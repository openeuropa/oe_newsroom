...
class NodeSubscriptionService {
    ...
    return array_map(
      function (array $entry) {
        ...
        if ((string) (int) $id !== (string) $id) {
          throw new OperationError(sprintf("Unexpected value %d for node id found in the response data.", var_export($id, TRUE)));
        }
      }, $response[0]['subscribedNotificationTopicType'] ?? [],
    );
  }
  ...
  public function getFrequencyOptions(callable $t) {
    return [
      ...
      NotificationFrequency::WEEKLY->getCode() => $this->t('Weekly'),
    ];
  }
